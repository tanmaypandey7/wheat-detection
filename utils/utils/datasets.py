    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            if random.random() < 0.5:
                img, labels = load_mixup_images_and_boxes(self, index)
            else:
...
def load_mixup_images_and_boxes(self, index):

    images, boxes = load_image_and_boxes(self, index)

    s_index = random.randint(0, len(self.labels) - 1)
    shuffled_images, shuffled_bboxes= load_image_and_boxes(self, s_index)
    ...
    mixedup_images = mixedup_images.astype(int)
    mixedup_bboxes = np.concatenate((boxes,shuffled_bboxes), 0)
    mixedup_images = np.uint8(mixedup_images)
    # show_result(mixedup_images, mixedup_bboxes)
    # plt.show()
    return mixedup_images, mixedup_bboxes


def show_result(sample_id, gt_boxes):
    ...
    ax.set_axis_off()
    ax.imshow(sample_id)
    ax.set_title("BLUE - Ground-truth")
